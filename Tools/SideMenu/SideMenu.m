...
#error "SideMenu requires ARC support."
#endif
...
-(instancetype)initWithItemTitles:(NSArray *)itemsTitle addToViewController:(UIViewController *)sender {
    ...
    if ((self = [super init])) {
        ...
        for(int i = 0;i<[itemsTitle count]; i++){
            MenuItem *temp = [[MenuItem alloc]initWithTitle:[itemsTitle objectAtIndex:i]
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self.delegate SideMenu:self didSelectItemAtIndex:indexPath.row];
    [self.delegate SideMenu:self selectedItemTitle:[titleArray objectAtIndex:indexPath.row]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MenuItem *item = [itemsArray objectAtIndex:indexPath.row];
