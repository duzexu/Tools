...
@interface ADImplemertorContext : NSObject

@property (nonatomic, weak) id implemertor;

@end

@implementation ADImplemertorContext

@end


@interface ADProtocolDispatcher ()
...
@implementation ADProtocolDispatcher

+ (id)dispatcherProtocol:(Protocol *)protocol toImplemertors:(NSArray *)implemertors {
    return [[ADProtocolDispatcher alloc] initWithProtocol:protocol toImplemertors:implemertors];
}
...
- (instancetype)initWithProtocol:(Protocol *)protocol toImplemertors:(NSArray *)implemertors {
    if (self = [super init]) {
        ...
        [implemertors enumerateObjectsUsingBlock:^(id implemertor, NSUInteger idx, BOOL * _Nonnull stop) {
            ADImplemertorContext *implemertorContext = [ADImplemertorContext new];
...
- (BOOL)respondsToSelector:(SEL)aSelector {
    ...
    for (ADImplemertorContext *implemertorContext in self.implemertors) {
